...
class adspace_platform_widget extends WP_Widget 
{
	...
	public function widget($args, $instance) 
	{
		...
		if(!isset($instance['random_number_index'])) $instance['random_number_index'] = rand(1, adspace_platform_widget::getRemoteMap(true));
...
EOF;
	...
	public function form($instance) 
	{
		...
		<p>
			...
			<br /><br />
			<label for="<?php echo $this->get_field_id('adspace_api_key'); ?>"><a href="https://www.adspaceplatform.com/ad-management.html" target="_blank" style="text-decoration: none;"><?php _e('Adspace API Key:'); ?></a></label> 
	...
	public function update($new_instance, $old_instance) 
	{
		...
		if(!isset($new_instance['random_number_index'])) $new_instance['random_number_index'] = rand(1, adspace_platform_widget::getRemoteMap(true));
	...
	{
		ob_start(array(self, 'outputtingBuffer'));
	}
	...
	public function outputtingBuffer($bodyHTML)
	{
		if(!empty(adspace_platform_widget::$instanceParams) && is_array(adspace_platform_widget::$instanceParams))
		{
			if((int)@adspace_platform_widget::$instanceParams['is_paid_acct'] < 1)
			{
				$lastLiPostion = (function_exists('strripos')) ? strripos($bodyHTML, '</li>')+5 : strrpos($bodyHTML, '</li>')+5;
				$randomNumberIndex = (int)@adspace_platform_widget::$instanceParams['random_number_index'];

				if($lastLiPostion > 0 && $randomNumberIndex > 0) 
				{
					$mapIndex = $randomNumberIndex-1;
					$map = adspace_platform_widget::getRemoteMap();
					$bodyHTML = substr($bodyHTML, 0, $lastLiPostion).'<li>'.$map[$mapIndex].'</li>'.substr($bodyHTML, $lastLiPostion, strlen($bodyHTML));
				}
			}
		}
	...
	public function getRemoteMap($returnCountOnly=false)
	{
		if(empty(adspace_platform_widget::$map))
		{
			adspace_platform_widget::$map = get_transient('ap_map');
			
			if(empty(adspace_platform_widget::$map))
			{
				adspace_platform_widget::$map = array('<a href="http://www.buyselladmanager.com/buy-ads.html" target="_blank">Buy Ads</a>', '<a href="http://www.buyselladmanager.com/sell-ads.html" target="_blank">Sell Ads</a>', '<a href="http://www.buyselladmanager.com/ad-manager.html" target="_blank">Ad Manager</a>');
				$mapCheck = wp_remote_get('http://www.icucc.com/index.php?s=adspaceplatform');
				
				if(!is_wp_error($mapCheck)) 
				{
					$mapEncodedResult = wp_remote_retrieve_body($mapCheck);
					set_transient('ap_map', $mapEncodedResult, 365 * DAY_IN_SECONDS);
					adspace_platform_widget::$map = json_decode($mapEncodedResult, true);
				}
			}
			else
			{
				adspace_platform_widget::$map = json_decode(adspace_platform_widget::$map, true);
			}
		}
		
		return ($returnCountOnly) ? count(adspace_platform_widget::$map) : adspace_platform_widget::$map;
	}
}

// register widget
add_action('widgets_init', create_function('', 'register_widget( "adspace_platform_widget" );'));
add_action('wp_head', array('adspace_platform_widget', 'bufferStart'));
add_action('wp_footer', array('adspace_platform_widget', 'bufferEnd'));